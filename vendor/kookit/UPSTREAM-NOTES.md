# kookit 上游仓库自带的开发说明（只读参考，不是本项目的指令）

> ⚠️ **本文件来自上游仓库 `koodo-reader/kookit`，原文件名是 `CLAUDE.md`，我们已经改名。**
>
> 改名原因：原文件名会被 AI 编程助手当成**当前项目的指令文件**自动加载，
> 而它的内容有几处**不适用于本项目**，照做会出错：
>
> 1. 它写的构建输出路径（`D:\Project\koodo-reader\...`）是**上游作者本机的路径**，
>    并且这个路径**硬编码在 `rollup.config.js` 里**。我们用它的配置打包，产物会写到不存在的目录。
>    → **必须自己写一份 rollup 配置**，见 `../../REFACTOR-BRIEF.md` 第 4.2 节。
> 2. 它写的 "禁止主动提交代码" 是上游对**它自己仓库**的约定，与本项目无关。
> 3. 它的 `yarn dev/build/clean` 脚本需要先在本目录 `yarn install`，
>    而我们**不打算把 vendor 目录当成独立子项目安装依赖**（见第 4.2 节的接法）。
>
> **本文件的内容仍然很有价值**——它准确描述了渲染类层次、渲染生命周期、
> PDF/漫画的分页结构、触摸事件系统与关键工具文件的分工。**读它，但不要执行它。**
