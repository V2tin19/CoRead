...
export const confirmBrowserExtensionAsync = async (): Promise<boolean> => {
  ...
  if (!result) {
    // 上游的 use-extension 文档页已不再指过去，统一回项目主页
...
export const handleFullScreen = () => {
  if (isElectron) {
    ...
    // Android Chrome 的元素全屏会自动把方向锁成横屏,
    // 稍后解锁让全屏跟随设备实际方向(竖屏手机保持竖屏)
...
export const getPageWidth = (
  ...
  if (readerMode === "scroll" || readerMode === "single") {
    const narrowScreen = document.body.clientWidth < 570;
    if (narrowScreen) {
      // 手机:页面宽度只由「屏幕宽度 - 页边距」决定,两侧各让出 margin px。
      // 页边距滑杆(-40~80)在这里才真正起作用:0 = 铺满,正值留白,负值继续外扩
      // (外扩超屏宽时按铺满处理)。缩放不改变页面宽度,只放大文字。
      const safeMargin = isFinite(margin) ? margin : 0;
      let width = document.body.clientWidth - 6 - 2 * safeMargin;
      width = Math.min(width, document.body.clientWidth - 6);
      width = Math.max(width - (width % 12), 120);
      pageOffset = `calc(50vw - ${width / 2}px)`;
      pageWidth = width;
    } else {
      // 桌面 single/scroll 保留 40% 视口的舒适留白
      let preWidth =
        document.body.clientWidth * Math.abs(parseFloat(scale)) -
        document.body.clientWidth * 0.4 -
        (isNavLocked ? 300 : 0) -
  ...
  if (pageWidth > document.body.clientWidth) {
    // 窄屏(手机):页面永远铺满屏幕,缩放交给 kookit 内部(文字变大,页面不变窄)。
    // 遗留逻辑会把页面压成 clientWidth - 106 并左右各留 50px —— 用户只要把
    // 页面缩放调过 1.0(哪怕 1.1),正文就被挤到约 73% 宽,两侧大片空白。
    if (document.body.clientWidth < 570) {
      const width = document.body.clientWidth - 6;
      pageWidth = width - (width % 12);
 ...
 * CoRead 项目主页（唯一真源）。
 *
 * 本项目是 Koodo Reader 的定制分支，这里刻意**不再指向上游官网** koodoreader.com / .cn：
 *   1. 那是另一款产品的官网，点过去看到的是与我们无关的品牌、定价和下载包；
 *   2. 上游 /pricing、/tts-quota 这类页面会把本机设备指纹（device_uuid）拼进 URL 带出去；
 *   3. 我们的文档与发布包统一放在 GitHub。
 *
 * 常量名保留（调用方遍布几十处），值统一切到 GitHub 仓库首页。
 */
...
/**
 * 上游的「升级 Pro / 购买额度」页面已不再跳转。
 *
 * 原实现会先向 api.koodoreader.com 取 temp_token、再拼上本机设备指纹，
 * 最后打开 koodoreader.com 的付费页。现在统一改为打开项目主页。
 */
export const openUpgradePage = () => {
  openExternalUrl(PROJECT_URL);
};
